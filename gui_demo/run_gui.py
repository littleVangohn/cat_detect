...
class CatFaceService:
    ...
    def _build_default_gallery(self) -> list[GalleryItem]:
        ...
        if candidates:
            extracted, body_extracted = self._extract_primary_modalities(
    ...
    def register(self, identity: str, paths: list[Path]) -> None:
        ...
        with self._inference_lock:
            inferred, body_inferred = self._extract_primary_modalities(resolved)
        ...
        try:
            while self.running:
                ...
                if now - self._last_detect >= self.PREVIEW_MIN:
                    ...
                    try:
                        matches, timing = self.service.identify_frame(frame, adaptive=self._adaptive)
                        self._last_timing = timing
                    except Exception as error:
                        self.root.after(0, lambda captured=error: self.status_var.set(f"识别出错：{captured}"))
                        matches, timing = [], Timing(total_ms=0.0)
                        self._last_timing = timing
